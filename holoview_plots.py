...
def create_age_distribution_focus(characters, age, sex, **kwargs):
    ...
    def map_characters_to_bins(characters, bin_edges):
        ...
        char_bins = {
            i: characters[(characters['age'] >= bin_edges[i]) & (characters['age'] <= bin_edges[i + 1])]['name'].tolist()
    ...
    hist_male_plot = hv.Histogram((bin_edges, hist_male)).relabel('Male').opts(alpha=0.3, color='blue', tools=["hover"])
    hist_female_plot = hv.Histogram((bin_edges, hist_female)).relabel('Female').opts(alpha=0.3, color='red',
                                                                                     tools=["hover"])
